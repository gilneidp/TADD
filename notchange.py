  ...
  def _handle_PacketIn (self, event):
    ...
    # Verificar se e ataque 
      ...
      for flow in tempd:
        ...
        print counter1
      if (counter1 > 5): # verifica se ha varias tentativas na mesma porta
     	attack = 1
        rt = RuleTable(id_switch=switches, switchport = inport, ip_src = packet.next.srcip,
		 ip_dst = packet.next.dstip, src_port = sport, dst_port = dport, timestamp = datetime.datetime.now(), idle_timeout=3000, hard_timeout=20000, action='mod_nw_dst:10.0.0.2')
        rt.save()
      else:
        attack = 0
